...
import com.pmc.ccms.config.EmbeddedKafka;
import com.pmc.ccms.config.KafkaSseConsumer;
import com.pmc.ccms.config.KafkaSseProducer;
...
class CcmsKafkaResourceIT {
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/ccms-kafka/publish?message=value-produce")).andExpect(status().isOk());
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/ccms-kafka/register"))
        ...
        for (int i = 0; i < 100; i++) {
            ...
            if (content.contains("data:value-consume")) {
                restMockMvc.perform(get("/api/ccms-kafka/unregister"));
                return;
            }
        }
